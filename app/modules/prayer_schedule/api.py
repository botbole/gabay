...
router = APIRouter(prefix="/synagogue", tags=["prayer_schedule"])
...
async def create_prayer_rule(req: PrayerRuleCreate):
    try:
...
async def update_prayer_rule(rule_id: str, req: PrayerRuleUpdate):
    try:
        fields = req.model_dump(exclude_unset=True)
        if 'days_of_week' in fields:
            fields['days_of_week'] = _days_to_str(fields['days_of_week'])
        rule = prayer_schedule_service.update_rule(rule_id, **fields)
...
async def delete_prayer_rule(rule_id: str):
    try:
        data = prayer_schedule_service.delete_rule(rule_id)
...
async def reorder_prayer_rules(req: ReorderRequest):
    try:
        data = prayer_schedule_service.reorder_rules(req.day_type, req.ordered_ids)
...
async def create_special_day(req: SpecialDayCreate):
    try:
...
async def delete_special_day(day_id: str):
    try:
        data = prayer_schedule_service.delete_special_day(day_id)
