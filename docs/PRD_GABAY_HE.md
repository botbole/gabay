...
גבאי הוא **מוצר מסחרי** הפועל על פני בתי כנסת מרובים. קיימות ארבע שכבות של משתמשים, לכל אחת תחום גישה מוגדר:

```
שכבה 0 – תמיכה / Super-Admin   (צוות המוצר גבאי)
שכבה 1 – Admin                  (מנהל בית הכנסת / גבאי ראשי)
שכבה 2 – Gabai                  (גבאי שוטף, 1–3 לבית כנסת)
שכבה 3 – מתפלל / Congregant    (שירות עצמי WhatsApp, v3.5)
```

### תמיכה / Super-Admin (שכבה 0)
- צוות המוצר גבאי. מספק תמיכה מרחוק לכל ההתקנות.
- גישה לפאנל פלטפורמה נפרד (`/platform`) — לא לממשק בית הכנסת הרגיל.
- יכול לצפות ולערוך את `TenantConfig` של כל בית כנסת לצורכי תמיכה.
- מנהל קטלוג מודולים — מה כלול במוצר הבסיסי ומה הוא תוסף.
- נדרש לפני שבית כנסת שני עולה לאוויר.

### מנהל מערכת (Admin — שכבה 1)
- איש הקשר הראשי של בית הכנסת: גבאי ראשי, יו"ר הנהלה, או נציג טכני.
- מנהל אחד בלבד לכל התקנה.
- מבצע Bootstrap ראשוני (אשף הגדרה או CLI), מגדיר הגדרות, ומנהל משתמשי גבאי.
- גישה לדף הגדרות (`/settings`): פרופיל בית הכנסת, מיקום, הגדרות LLM, ניהול משתמשים, עזרה ותמיכה.
- בנוסף לגישה המנהלתית — גישה תפעולית מלאה.
- היחיד שרשאי לשנות שדות `TenantConfig` או לנהל Roles בתוך בית הכנסת שלו.

### גבאי (Gabai — שכבה 2)
- המשתמש התפעולי העיקרי. ייתכנו 1–3 גבאים בהרשאות שוות.
- מנהל מתפללים, תשלומים, עליות, מקומות, אזכרות, שמחות, לוח שבועי, לוח תפילות, ייבוא, דוחות וצ'אט LLM.
- עובד בעברית וזקוק לגישה מהירה בזמן תפילה.
- **אינו** רואה את דף ההגדרות. אינו מנהל משתמשים ואינו משנה הגדרות מערכת.

### מתפלל (Congregant — שכבה 3)
- משתמש ב-WhatsApp למידע ציבורי ולשירות עצמי אישי; אין צורך בהרשמה, אפליקציה או `User` להתחברות.
- מזוהה לפי מספר טלפון מאומת שמותאם לרשומת `Congregant`.
- רשאי לקרוא רק תשלומים, עליות, תזכורות ונתונים אחרים ששייכים לו במפורש.
- יכול לבצע עדכונים עצמיים בטוחים מיד; שינוי רגיש בזהות, כספים, אזכרות, מקומות או הרשאות הופך לבקשה הדורשת אישור גבאי.

| יכולת | super_admin | admin | gabai | congregant |
|---|---|---|---|---|
| פאנל פלטפורמה (כל בתי הכנסת) | מלא | ללא | ללא | ללא |
| דף הגדרות (בית הכנסת שלו) | מלא | מלא | ללא | ללא |
| ניהול משתמשים ו-Roles | מלא | בית הכנסת שלו | ללא | ללא |
| כל המודולים התפעוליים | מלא | מלא | מלא | ללא |
| דוחות וייבוא | מלא | מלא | מלא | ללא |
| כלי LLM | הכל | מנהלתי + תפעולי | תפעולי | ציבורי + `my_*` |
| נתונים אישיים | כל הרשומות | כל הרשומות | כל הרשומות | עצמו בלבד |
...
רשימת הכלים בפועל מסוננת בצד השרת לפי Role ו-Scope: מנהל מערכת מקבל כלים מנהליים ותפעוליים, גבאי מקבל כלים תפעוליים, ומתפלל מקבל רק כלים ציבוריים וכלי `my_*`. כל קריאת כלי עוברת אימות הרשאה נוסף בשכבת Service/DB.
...
### 5.5 הגדרות מערכת (v2.5)

**מטרה:** ממשק מרכזי למנהל המערכת לכל הגדרה חוצת-מודולים.

**לשוניות דף ההגדרות (מנהל בלבד):**
- **פרופיל בית הכנסת** — שם, לוגו, צבעי מיתוג
- **מיקום ושעות תפילה** — עיר ו-Geoname ID (מחשב את כל זמני התפילה); תצוגה מקדימה של הדלקת נרות
- **עוזר AI** — ספק (OpenAI / Azure / Ollama), מודל, מפתח API (masked), Base URL, כפתור "בדוק חיבור"
- **משתמשים** — טבלת גבאים: שם, תפקיד, כניסה אחרונה, סטטוס; הזמנה, השבתה, שינוי תפקיד
- **עזרה ותמיכה** — גרסת האפליקציה, קישור ל-Changelog, תיעוד, ערוץ תמיכה

**ניידות נתונים:**
- ייצוא כל נתוני בית הכנסת כ-JSON — מלשונית עזרה ותמיכה. בתי הכנסת הם הבעלים של הנתונים שלהם.

**Banner Onboarding:**
- כניסת מנהל בפעם הראשונה מפעילה סרגל הדרכה עד ש-`TenantConfig.setup_completed = true`.

### 5.6 פריסה לייצור (v3.0)
- **אבטחה:** HSTS, CSP, לוגים מובנים, Rate limiting, ולידציית JWT בייצור
- **Docker:** Dockerfiles רב-שלביים ל-Backend ו-Frontend; `docker-compose.yml` עם health checks
- **מעבר ל-PostgreSQL:** סקריפט מיגרציה SQLite → PostgreSQL עם dry-run ואימות
- **ענן:** AWS ECS Fargate + RDS PostgreSQL; Secrets Manager לאישורים

### 5.7 התקנה ו-Onboarding (v3.1)

**מטרה:** להפוך את גבאי לניתן להתקנה ולהפעלה על-ידי מנהל בית כנסת שאינו מפתח.

- **`docs/INSTALLATION.md`** — צעד-אחר-צעד מ-`git clone` עד כניסה ראשונה (פחות מ-30 דקות)
- **`docs/UPGRADE.md`** — גיבוי, משיכה, מיגרציה, אימות; שדרוגים ללא downtime
- **`.env.example`** — כל משתנה מתועד בעברית עם ברירות מחדל ו-placeholders
- **`CHANGELOG.md`** — מתוחזק עם כל גרסה; מקושר מלשונית עזרה ב-Settings
- **Bootstrap CLI** — `python -m app.cli bootstrap` יוצר את משתמש ה-admin הראשון בשיח
- **אשף First-Run** — אם אין admin ב-DB, האפליקציה מפנה לאשף הגדרה: שם בית כנסת → עיר → סיסמת admin → (אופציונלי) הזמנת גבאי

### 5.8 פלטפורמת תמיכה (v3.2)

**מטרה:** לתת לצוות המוצר גבאי ראות ושליטה על כל ההתקנות ללא גישת SSH.

- **נתיב `/platform`** — נגיש ל-`super_admin` בלבד; לא מופיע בסרגל הניווט הרגיל
- **לוח מחוון Tenants** — כל בתי הכנסת: שם, עיר, גרסה, כניסה אחרונה, סטטוס setup
- **Config מרחוק** — צפייה ועריכה של `TenantConfig` של כל בית כנסת לתמיכה
- **קטלוג מודולים** — רשימה סמכותית של כל המודולים, הטייר שלהם (base / addon / enterprise) וסט ברירת המחדל להתקנות חדשות. `super_admin` מנהל זאת; מתפתח למערכת הרישיונות ב-v4.0.
- **Audit Log** — כל פעולת admin נרשמת: actor, פעולה, ישות, ערכים לפני/אחרי, timestamp
- **התראות in-app לאדמין:** כשל LLM, לא בוצע export ב-30 יום, גרסה חדשה זמינה
- **Feedback in-app:** כפתור "דווח על בעיה" (בסרגל, נגיש לגבאים) שולח ל-webhook/מייל

### 5.9 בוט וואטסאפ קהילתי (v3.5)
> **תלוי ב-v3.0** – נדרש JWT Auth ומודל LLM Scope לפני פיתוח הבוט.
- **ממשק מתפלל:** שירות עצמי 24/7 דרך WhatsApp, ללא הרשמה, אפליקציה או `User` להתחברות
- **זיהוי לפי טלפון:** טלפון מאומת → `Congregant` → `congregant_id` שנאכף בשרת
- **תהליך אישור:** שינוי רגיש מועבר לאישור גבאי
- **Broadcast:** שליחת עדכונים לכלל הקהילה מממשק הגבאי

### 5.10 לוח שבועי מעוצב (v3.6)
- מנוע Canva-style עם Auto-Scaling לדף A4, ערכות נושא
- ייצוא PDF ותמונה (מוכנה לוואטסאפ)

### 5.11 ארכיטקטורת SaaS (v4.0)
- **Multi-tenancy:** `tenant_id` על כל המודלים; ניתוב לפי subdomain; הפרדת נתונים
- **מערכת רישיונות:** מודל `License` — תוכנית (Basic / Premium / Enterprise), תפוגה, מודולים מורשים
- **קטלוג מודולים מתפתח:** `tier` ממודל קטלוג המודולים (v3.2) הופך לנקודת ייחוס לרישוי
- **פאנל super-admin מורחב:** חיוב, הענקת הרשאות מודולים לפי Tenant, Onboarding לבתי כנסת חדשים

### 5.12 אפליקציית מובייל – Android ו-iOS (v5.0)
...
| **דף הגדרות + ניידות נתונים + Banner onboarding** | **v2.5** |
| אבטחה + Docker + פריסה לענן | v3.0 |
| **תיעוד התקנה + אשף First-Run + CHANGELOG** | **v3.1** |
| **פלטפורמת תמיכה + Audit Log + Feedback + קטלוג מודולים** | **v3.2** |
| בוט וואטסאפ קהילתי *(תלוי ב-v3.0)* | v3.5 |
| לוח שבועי מעוצב (Canva-style) | v3.6 |
| SaaS Multi-tenancy + מערכת רישיונות | v4.0 |
...
3. **אימות והרשאות:** תשתית JWT/sessions, הגנת Routes/Services ו-Role מסוג `gabai` הושלמו ב-Backend. מסך Login, UI לפי Role ואכיפת Scope ב-LLM/WhatsApp נותרו ל-v3.0 ומהווים תנאי מוקדם לבוט WhatsApp (v3.5).
