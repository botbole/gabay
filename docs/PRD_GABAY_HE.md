...
### מנהל מערכת (Admin)
- מאתחל את חשבון בית הכנסת ומנהל משתמשי Web ושיוך Roles
- מגדיר את הגדרות בית הכנסת (`TenantConfig`), מודולים פעילים, אינטגרציות ואבטחה
- מחזיק בגישה תפעולית לשעת חירום, אך העבודה השוטפת מיועדת בדרך כלל לגבאי
- היחיד שרשאי לשנות הגדרות בית כנסת/מערכת מוגנות

### גבאי (Gabai)
- המשתמש התפעולי העיקרי: גבאי ראשי, סגן או מנהל כספים/קהילה מורשה
- מנהל מתפללים, תשלומים, עליות, מקומות, אזכרות, שמחות, ייבוא, דוחות, תקשורת וכלי LLM תפעוליים
- עובד בעברית וזקוק למידע מהיר בזמן תפילה
- אינו מנהל משתמשים או Roles ואינו משנה הגדרות בית כנסת/מערכת מוגנות
...
תהליכי רב/חזן לקריאה בלבד יוכלו לקבל Role מוגבל בעתיד; הם אינם Role נפרד במודל היעד הנוכחי בן שלושת התפקידים.

מודל הפרסונות מיושם ב-Backend: ממשקי התפעול זמינים ל-`admin` ול-`gabai`, בעוד שניהול משתמשים והגדרות מוגנות נשארים ל-`admin` בלבד.
...
- **זיהוי לפי טלפון:** טלפון מאומת → `Congregant` → `congregant_id` שנאכף בשרת; שולח לא מוכר או עמום אינו מקבל מידע פרטי
- **פעולות מיידיות:** מידע ציבורי, קריאת מידע אישי ועדכונים עצמיים שהוגדרו בטוחים
- **תהליך אישור:** שינוי רגיש בזהות, כספים, אזכרות, מקומות או הרשאות מועבר לאישור גבאי
- **Audit:** זיהוי, Scope, כלים, שינויים, אישורים ותוצאות נרשמים ביומן ביקורת
