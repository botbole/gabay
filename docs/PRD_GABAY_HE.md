...
### 5.1 תקשורת ולוח שבועי
- **מרכז תקשורת:** שליחת תזכורות אוטומטיות בוואטסאפ ומייל לאזכרות ושמחות
- **לוח שבועי דינמי (2.5):** מחולל אוטומטי של הודעת שבת מעוצבת לוואטסאפ, מייל (Google Groups), והדפסה בסיסית
- **לוח שבועי מעוצב (2.9):** מנוע עיצוב Canva-style עם Auto-Scaling לדף A4, ניהול ערכות נושא (Themes) וייצוא PDF/תמונה איכותי

### 5.2 פיננסים ודוחות
- **ניהול פיננסי מורחב (2.7):** תיעוד הוצאות והכנסות שאינן תרומות (השכרת אולם, מענקים)
- **דוח שנתי:** ייצוא P&L (הכנסות מול הוצאות) לוועד בית הכנסת
- **קבלות PDF:** הפקת קבלות מעוצבות עם לוגו בית הכנסת

### 5.3 בינה מלאכותית מתקדמת
- **LLM 2.0 (2.5):** תמיכה ב-RAG על מסמכי PDF (תקנון, נהלים, הלכות), שאילתות מורכבות
- **שיבוץ עליות חכם (2.8):** הצעת שיבוצים אוטומטית בהתבסס על תדירות, יארצייט וסטטוס כהן/לוי/ישראל
- **לוח זמנים מבוסס כללים (2.6):** חישוב זמני תפילות אוטומטי ביחס לשקיעה/הנץ

### 5.4 בוט וואטסאפ קהילתי (3.5)
...
### 5.6 אפליקציית מובייל – Android ו-iOS (Milestone 6)
...
| פיצ'ר | סטטוס |
|---|---|
| ניהול משתמשים / הרשאות (JWT) | Milestone 3 |
| לוח שבועי דינמי | Milestone 2.5 |
| לוח שבועי מעוצב (Canva-style) | Milestone 2.9 |
| ניהול פיננסי ודוחות שנתיים | Milestone 2.7 |
| שיבוץ עליות חכם + מלאי | Milestone 2.8 |
| בוט וואטסאפ קהילתי | Milestone 3.5 |
| ארכיטקטורת פלטפורמה (SaaS) | Milestone 4 |
| בדיקות E2E (Playwright) | Milestone 5 |
| אפליקציית מובייל (Android + iOS) | Milestone 6 |
...
2. **גיבוי נתונים:** מתוכנן ב-Milestone 3 – endpoint ייצוא CSV ותיעוד גיבוי ידני של `gabay.db`.
3. **אימות:** מתוכנן ב-Milestone 3 – JWT + Login page מלא.
4. **שפת UI:** הממשק עבר לעברית מלאה (RTL) – כל הכותרות, הכפתורות והתפריטים בעברית.
5. **מובייל:** פיתוח האפליקציה ב-React Native מתוכנן ל-Milestone 6 לאחר יציבות ה-API.
